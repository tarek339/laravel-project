...
test('check:trucks command displays trucks next tachograph inspection dates', function () {
    ...
    $truck1 = Truck::factory()->create([
        ...
        'next_tachograph_inspection' => now()->addDays(60),
    ]);
    ...
    $this->artisan('app:check-trucks-command')
        ->expectsOutput("Truck {$truck1->license_plate} next tachograph inspection is in two months - {$truck1->next_tachograph_inspection}")
        ->expectsOutput("Truck {$truck2->license_plate} next tachograph inspection is in two months - {$truck2->next_tachograph_inspection}")
        ->assertExitCode(0);

    // Test the data separately by querying the database directly
    $nextTachographInspectionOneMonth = Truck::whereDate('next_tachograph_inspection', '=', now()->addDays(60)->toDateString())->get();
