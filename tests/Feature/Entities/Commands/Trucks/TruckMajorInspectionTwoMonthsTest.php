...
test('check:trucks command displays trucks next major inspection dates', function () {
    ...
    $this->artisan('app:check-trucks-command')
        ->expectsOutput("Truck {$truck1->license_plate} next major inspection is in two months - {$truck1->next_major_inspection}")
        ->expectsOutput("Truck {$truck2->license_plate} next major inspection is in two months - {$truck2->next_major_inspection}")
        ->assertExitCode(0);

    // Test the data separately by querying the database directly
    $nextMajorInspectionOneMonth = Truck::whereDate('next_major_inspection', '=', now()->addDays(60)->toDateString())->get();
