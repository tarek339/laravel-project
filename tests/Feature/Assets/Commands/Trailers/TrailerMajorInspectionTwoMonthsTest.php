...
test('check:trailers command displays trailers next major inspection dates', function () {
    ...
    $trailer1 = Trailer::factory()->create([
        ...
        'next_major_inspection' => now()->addDays(60),
    ]);
    ...
    $this->artisan('app:check-trailers-command')
        ->expectsOutput("Trailer {$trailer1->license_plate} next major inspection is in two months - {$trailer1->next_major_inspection}")
        ->expectsOutput("Trailer {$trailer2->license_plate} next major inspection is in two months - {$trailer2->next_major_inspection}")
        ->assertExitCode(0);

    // Test the data separately by querying the database directly
    $nextMajorInspectionOneMonth = Trailer::whereDate('next_major_inspection', '=', now()->addDays(60)->toDateString())->get();
